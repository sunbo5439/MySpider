...
class TOM61(scrapy.Spider):
    ...
    def parse_1(self, response):
        a_list= response.xpath('//div[@class="t_auto pb16"]/ul/li/a')
        for a in a_list:
    ...
    def parse_2(self, response):
        ...
        a_list= response.xpath('//div[@class="t_auto pb16"]/dl[@class="txt_box"]/dd/a')
        for a in a_list:
            url=response.urljoin( a.xpath('./@href').extract_first() )
            request = scrapy.Request(url,callback=self.parse_3)
            request.meta['category'] = category
            yield request
        next_page= response.xpath('//div[@class="t_fy"]/a[@class="nextpage"]')
        if next_page is not None:
            url=next_page.xpath('./@href').extract_first()
            next_page = response.urljoin(url)
            request =scrapy.Request(next_page, callback=self.parse_2)
            request.meta['category'] = category
            yield request
...
class GZ_YL(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        start_urls=['http://www.51test.net/channel_list_more.asp?page=1&classid=17&Nclassid=197&Key=&search_key=%D2%E9%C2%DB%CE%C4&search_key2=%B8%DF%D2%BB',
